...
class Mamba2Simple(nn.Module):
    def __init__(
        ...
        layer_idx=None,  # Absorb kwarg for general modules
        device=None,
        dtype=None,
    ):
    ...
    def forward(self, u, seq_idx=None, local_init=None):
        ...
        # V1
        # gates = torch.sigmoid(self.gate(u))
        # softmax = F.softmax(u, dim=-1)
        # out = out+gates*softmax
        ...
        # softmax = F.softmax(u, dim=-1)
        # out = out * softmax
        # out = self.gatenorm(out)

        # V4
        # softmax = F.softmax(u, dim=-1)
        # out = out + u + self.alpha * softmax
        ...
        # V6:FFT
        # 进行快速傅里叶变换
        # fft_result = torch.fft.fft(u)
        # fft_result_real = fft_result.real
        # out = out + self.gatenorm(fft_result_real)

        # V7:FFT+u
        # fft_result = torch.fft.fft(u)
        # fft_result_real = fft_result.real
        # out = out + self.gatenorm(fft_result_real) + u
