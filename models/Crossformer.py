...
class MTSModel(nn.Module):
    def __init__(self, args):
        ...
        self.pad_in_len = ceil(1.0 * args.in_len / args.seg_len) * args.seg_len
        self.pad_out_len = ceil(1.0 * args.out_len / args.seg_len) * args.seg_len
        self.in_len_add = self.pad_in_len - self.in_len

        # Embedding
        self.enc_value_embedding = DSW_embedding(args.seg_len, args.d_model)
        self.enc_pos_embedding = nn.Parameter(torch.randn(1, args.data_dim, (self.pad_in_len // args.seg_len), args.d_model))
        self.pre_norm = nn.LayerNorm(args.d_model)

        # Encoder
        self.encoder = Encoder(args.e_layers, args.win_size, args.d_model, args.n_heads, args.d_ff, block_depth=1, \
                               dropout=args.dropout, in_seg_num=(self.pad_in_len // args.seg_len), factor=args.factor)

        # Decoder
        self.dec_pos_embedding = nn.Parameter(torch.randn(1, args.data_dim, (self.pad_out_len // args.seg_len), args.d_model))
        self.decoder = Decoder(args.seg_len, args.e_layers + 1, args.d_model, args.n_heads, args.d_ff, args.dropout, \
                               out_seg_num=(self.pad_out_len // args.seg_len), factor=args.factor)
